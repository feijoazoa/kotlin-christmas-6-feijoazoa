...
class Menu {
    ...
    private val mains =
        mapOf("티본스테이크" to 55000, "바비큐립" to 54000, "해산물파스타" to 35000, "크리스마스파스타" to 25000)
    ...
    fun isValidMenuOrder(order: String): Boolean {
        ...
        if (!isValidOrderSize(orderedItems, orderedItemNames)) return false
        if (!isValidItems(orderedItems)) return false

        return true
    }

    private fun isValidOrderSize(
        orderedItems: List<String>,
        orderedItemNames: List<String>
    ): Boolean {
        return orderedItems.size <= 20 && orderedItems.size == orderedItemNames.distinct().size && !orderedItemNames.all { it in drinks }
    }

    private fun isValidItems(orderedItems: List<String>): Boolean {
